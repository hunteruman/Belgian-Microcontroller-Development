...
% (c) 2019 - Laurens Jacobs, MECO Research Team @ KU Leuven
 
    %% Properties
    properties
        t % the time vector, N samples (1D matrix)
        x % the state vectors, N samples x nx number of states (2D matrix)
        P % the covariance matrices of the states, N samples x nx number of states x nx number of states (3D matrix)
        R % the covariance matrices of the measurements, N samples x ny number of measurements x ny number of measurements (3D matrix)
        y % the measurements, N samples x ny number of measurements (2D matrix)
        u % the inputs, N samples x nu number of inputs (2D matrix) 
        nu % the innovations, N samples x nx number of innovations (2D matrix) 
        S  % the innovation covariance, N samples x ny number of measurements x ny number of measurements (3D matrix) 
    end
    ...
    methods(Access=public) 
        ...
        %   R - the covariance matrices of the measurements, N samples x ny number of measurements x ny number of measurements (3D matrix)
        %   u - the inputs, N samples x nu number of inputs (2D matrix) 
        %   nu - the innovations, N samples x ny number of innovations (2D matrix)
        %   S - the innovation covariance matrix, N samples x ny number of innovations x ny number of innovations
        ...
        % 99% confidence interval of states 1 and 2.
            ...
            bound = [box(:,1)         obj.x+box(:,2:end)        ;
                     flipud(box(:,1)) flipud(obj.x-box(:,2:end))];
        ...
        % plots the 99% confidence interval of measurements 1 and 2.
            ...
            bound = [box(:,1)         obj.y+box(:,2:end)        ;
                     flipud(box(:,1)) flipud(obj.y-box(:,2:end))];
        ...
        % set 'log' as 'false'. 
            ...
            for i=1:length(idx)
                j = idx(i); 
                [V,D] = eig(squeeze(obj.P(j,states,states)));
        ...
        function [probNIS, probSNIS] = analyzeconsistency(obj, confidence, M)
        ...
        % the SNIS. 
            ...
            for i=1:obj.N()
                NIS(i) = obj.nu(i,:)/squeeze(obj.S(i,:,:))*obj.nu(i,:)';
            end
            for i=1:obj.N()-M+1
   ...
   %% Private helper methods
    methods(Access=private)
        ...
        % CHECKINPUT Checks the consistency of the input arguments
            ...
            assert(all(diff([size(obj.t,1), size(obj.x,1), size(obj.P,1), size(obj.R,1), size(obj.y,1), size(obj.u,1) size(obj.nu,1)])==0), 'All inputs, outputs and states should have the same number of time samples.');
            assert(all(diff([size(obj.x,2), size(obj.P,2), size(obj.P,3)])==0), 'State covariance matrix size does not match the number of states.');
            assert(all(diff([size(obj.y,2), size(obj.nu,2)])==0), 'Number of innovations does not match the number of measurements.');
            assert(all(diff([size(obj.y,2), size(obj.R,2), size(obj.R,3)])==0), 'Measurement covariance matrix size does not match the number of measurements.');
        end
        ...
        % axis is returned (positive). 
            ...
            alpha = chi2inv(confidence, obj.nx()); 
            box = zeros(obj.N(), obj.nx()+1);
            box(:,1) = obj.t; 
            for i=1:obj.N()
                P = inv(squeeze(obj.P(i,:,:))); 
                for j=1:obj.nx()
                    A = P; A(j,:) = []; A(:,j) = [];
                    b = P(:,j); b(j,:) = []; s = -A\b; 
                    sub = zeros(obj.nx(),1);
                    sub(1:j-1) = s(1:j-1); sub(j) = 1; sub(j+1:end) = s(j:end);
                    box(i,j+1) = sqrt(alpha/(sub'*P*sub));
                end
            end
        end
        ...
        % axis is returned (positive). 
            ...
            box = zeros(obj.N(), obj.nout()+1);
            box(:,1) = obj.t; 
            for i=1:obj.N()
                R = inv(squeeze(obj.R(i,:,:))); 
                for j=1:obj.nout()
                    A = R; A(j,:) = []; A(:,j) = [];
                    b = R(:,j); b(j,:) = []; s = -A\b; 
                    sub = zeros(obj.nout(),1);
                    sub(1:j-1) = s(1:j-1); sub(j) = 1; sub(j+1:end) = s(j:end);
                    box(i,j+1) = sqrt(alpha/(sub'*R*sub));
                end
            end
        end
    ...
    methods(Static)
        ...
        % assignments. 
            ...
            msg = [ '\fontsize{11}' ...
                    ...
                    '\bfNotes\rm:' nl ...
                    'If you forgot to set the correct labels and don''t want to redo your experiment, you can edit the labels manually by changing the first line of the CSV file in a text editor.' ];
            ...
            if ~evalin('base','exist(''R'',''var'')')
                msg = '\fontsize{11}The import was succesful! You now still have to provide the measurement covariance matrix. Click ''OK'' and type the covariance in the command window. If you define R in your workspace before running this function, this message will not appear anymore.';
                opts = struct('WindowStyle', 'replace', 'Interpreter', 'tex'); 
            ...
            R = permute(repmat(R,[1 1 size(readdata,1)]),[3 1 2]);
            ...
            % interested.
            u = zeros(size(readdata,1),0);
            
            % 5. Cast into a KalmanExperiment object
            obj = KalmanExperiment(readdata(:,1)/1000, readdata(:,3), readdata(:,2), readdata(:,4), R, u, readdata(:,5), readdata(:,6)); 
        ...
        % assignments. 
            ...
            labels = {'P','P12','P13','P22','P23','P33','x1','x2','x3','y1','y2'}; 
            ...
            msg = [ '\fontsize{11}' ...
                    ...
                    ' 3. If you forgot to set the correct labels and don''t want to redo your experiment, you can edit the labels manually by changing the first line of the CSV file in a text editor.' ];
            opts = struct('WindowStyle', 'replace', 'Interpreter', 'tex'); 
            ...
            % 3. Request the measurement noise matrix
            if ~evalin('base','exist(''R'',''var'')')
                msg = '\fontsize{11}The import was succesful! You now still have to provide the 2 x 2 measurement covariance matrix. Click ''OK'' and type the covariance matrix in the command window. If you define R in your workspace before running this function, this message will not appear anymore.';
                opts = struct('WindowStyle', 'replace', 'Interpreter', 'tex'); 
            ...
            % 5. Cast into a KalmanExperiment object
            P = permute(reshape([readdata(:,2:4) readdata(:,3) readdata(:,5:6) readdata(:,4) readdata(:,6) readdata(:,7)]', [3,3,size(readdata,1)]), [3 1 2]);
            obj = KalmanExperiment(readdata(:,1)/1000, readdata(:,2:4), P, readdata(:,11:12), R, u); 
